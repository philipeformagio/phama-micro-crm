...
using PhamaMicroCrm.Api.Controllers;
...
namespace PhamaMicroCrm.Api.V1.Controllers
{
    ...
    public class AuthController : MainController
    {
        ...
        public async Task<ActionResult> Login(LoginUserDto loginUser)
        {
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("User " + loginUser.Email + " successfully logged.");
                return CustomResponse(await GenerateJwt(loginUser.Email));
            }
            if (result.IsLockedOut)
            {
                NotificarErro("User temporarily blocked by invalid attempts.");
                return CustomResponse(loginUser);
            }

            NotificarErro("User ou Password incorretos");
            return CustomResponse(loginUser);
        }
        ...
        private async Task<LoginResponseViewModel> GenerateJwt(string email)
        {
            ...
            var token = tokenHandler.CreateToken(new SecurityTokenDescriptor
            {
                ...
                Audience = _appSettings.ExpiresIn,
